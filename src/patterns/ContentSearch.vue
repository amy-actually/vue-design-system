<template>
  <component :is="wrapper" class="input" style="min-width:250px">
    ...
    <div class="mt-3">
          ...
          <template v-for="parent in terms[taxonomy]">
            <li :key="parent.id">
              ...
              >
                ...
                ><span class="taglist__tag--name" v-html="parent.name"></span
              ></a>
              <ul v-if="parent.children && parent.children.length > 0" class="taglist__children">
                <li v-for="child in parent.children" :key="child.id">
                  <a
                    ...
                    :class="{
                      selected:
                        selected[taxonomy].includes(child.parent) ||
                        selected[taxonomy].includes(child.id),
                    }"
                  >
                    <span class="taglist__tag--close">&times;</span>
                    <span class="taglist__tag--name" v-html="child.name"></span
                  ></a>
                </li>
              </ul>
            </li>
      ...
      <button class="button button--blue-alternate" v-on:click="clearFilter">Clear Filter</button>
    </div>
  </component>
</template>
<script>
  ...
  methods: {
    ...
    selectTerms(tax, term) {
      ...
      if (this.selected[tax].includes(term.id) && (!term.children || term.children.length == 0)) {
        ...
        select.terms = this.selected[tax].filter(
          val => val != term.id && term.children.every(child => child.id != val)
        )
      } else {
